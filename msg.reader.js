 ...
 */
(function () {
  const FILE_HEADER = uInt2int([0xD0, 0xCF, 0x11, 0xE0, 0xA1, 0xB1, 0x1A, 0xE1]);
  const MSG = {
    UNUSED_BLOCK: -1,
    END_OF_CHAIN: -2,
  ...
  function isMSGFile(ds) {
    ds.seek(0);
    return arraysEqual(FILE_HEADER, ds.readInt8Array(FILE_HEADER.length));
  }
  ...
  function headerData(ds) {
    ...
    headerData.bigBlockSize =
      ds.readByte(/*const position*/30) === MSG.L_BIG_BLOCK_MARK ? MSG.L_BIG_BLOCK_SIZE : MSG.S_BIG_BLOCK_SIZE;
    headerData.bigBlockLength = headerData.bigBlockSize / 4;
    headerData.xBlockLength = headerData.bigBlockLength - 1;

    // header data
    headerData.batCount = ds.readInt(MSG.HEADER.BAT_COUNT_OFFSET);
    headerData.propertyStart = ds.readInt(MSG.HEADER.PROPERTY_START_OFFSET);
    headerData.sbatStart = ds.readInt(MSG.HEADER.SBAT_START_OFFSET);
    headerData.sbatCount = ds.readInt(MSG.HEADER.SBAT_COUNT_OFFSET);
    headerData.xbatStart = ds.readInt(MSG.HEADER.XBAT_START_OFFSET);
    headerData.xbatCount = ds.readInt(MSG.HEADER.XBAT_COUNT_OFFSET);

    return headerData;
  }

  function batCountInHeader(msgData) {
    var maxBatsInHeader = (MSG.S_BIG_BLOCK_SIZE - MSG.HEADER.BAT_START_OFFSET) / 4;
    return Math.min(msgData.batCount, maxBatsInHeader);
  }

  function batData(ds, msgData) {
    var result = new Array(batCountInHeader(msgData));
    ds.seek(MSG.HEADER.BAT_START_OFFSET);
  ...
  function sbatData(ds, msgData) {
    ...
    for (var i = 0; i < msgData.sbatCount && startIndex !== MSG.END_OF_CHAIN; i++) {
  ...
  function xbatData(ds, msgData) {
    ...
    for (var i = 0; i < msgData.xbatCount; i++) {
      ...
      for (var j = 0; j < blocksToProcess; j++) {
        ...
        if (blockStartAt === MSG.UNUSED_BLOCK || blockStartAt === MSG.END_OF_CHAIN) {
          break;
        }
  ...
  function propertyData(ds, msgData) {
    ...
    while (currentOffset !== MSG.END_OF_CHAIN) {
  ...
  function convertName(ds, offset) {
    const nameLength = ds.readShort(offset + MSG.PROP.NAME_SIZE_OFFSET);
    return nameLength < 1 ? '' : ds.readStringAt(offset, nameLength / 2);
  }

  function convertProperty(ds, index, offset) {
    return {
      index: index,
      type: ds.readByte(offset + MSG.PROP.TYPE_OFFSET),
      name: convertName(ds, offset),
      // hierarchy
      previousProperty: ds.readInt(offset + MSG.PROP.PREVIOUS_PROPERTY_OFFSET),
      nextProperty: ds.readInt(offset + MSG.PROP.NEXT_PROPERTY_OFFSET),
      childProperty: ds.readInt(offset + MSG.PROP.CHILD_PROPERTY_OFFSET),
      // data offset
      startBlock: ds.readInt(offset + MSG.PROP.START_BLOCK_OFFSET),
      sizeBlock: ds.readInt(offset + MSG.PROP.SIZE_OFFSET)
    };
  }

  function convertBlockToProperties(ds, msgData, propertyBlockOffset, props) {
    var propertyCount = msgData.bigBlockSize / MSG.PROP.PROPERTY_SIZE;
    var propertyOffset = getBlockOffsetAt(msgData, propertyBlockOffset);

    for (var i = 0; i < propertyCount; i++) {
      var propertyType = ds.readByte(propertyOffset + MSG.PROP.TYPE_OFFSET);
      switch (propertyType) {
        case MSG.PROP.TYPE_ENUM.ROOT:
        case MSG.PROP.TYPE_ENUM.DIRECTORY:
        case MSG.PROP.TYPE_ENUM.DOCUMENT:
      ...
      propertyOffset += MSG.PROP.PROPERTY_SIZE;
    }
  }

  function createPropertyHierarchy(props, nodeProperty) {
    if (nodeProperty.childProperty === MSG.PROP.NO_INDEX) {
      return;
    }
    ...
    while (children.length) {
      ...
      if (current.type === MSG.PROP.TYPE_ENUM.DIRECTORY) {
        createPropertyHierarchy(props, current);
      }
      if (current.previousProperty !== MSG.PROP.NO_INDEX) {
        children.push(current.previousProperty);
      }
      if (current.nextProperty !== MSG.PROP.NO_INDEX) {
  ...
  function fieldsDataDir(ds, msgData, dirProperty, fields) {
    if (dirProperty.children && dirProperty.children.length > 0) {
      for (var i = 0; i < dirProperty.children.length; i++) {
        ...
        if (childProperty.type === MSG.PROP.TYPE_ENUM.DIRECTORY) {
          fieldsDataDirInner(ds, msgData, childProperty, fields)
        } else if (childProperty.type === MSG.PROP.TYPE_ENUM.DOCUMENT && childProperty.name.indexOf(MSG.FIELD.PREFIX.DOCUMENT) === 0) {
  ...
  function fieldsDataDirInner(ds, msgData, dirProperty, fields) {
    if (dirProperty.name.indexOf(MSG.FIELD.PREFIX.ATTACHMENT) === 0) {
      // attachment
      var attachmentField = {};
      fields.attachments.push(attachmentField);
      fieldsDataDir(ds, msgData, dirProperty, attachmentField);
    } else if (dirProperty.name.indexOf(MSG.FIELD.PREFIX.RECIPIENT) === 0) {
      // recipient
      var recipientField = {};
      ...
      if (childFieldType !== MSG.FIELD.DIR_TYPE.INNER_MSG) {
  ...
  function fieldsDataDocument(ds, msgData, documentProperty, fields) {
    ...
    var fieldName = MSG.FIELD.NAME_MAPPING[fieldClass];
    var fieldTypeMapped = MSG.FIELD.TYPE_MAPPING[fieldType];
    ...
    if (fieldClass === MSG.FIELD.CLASS_MAPPING.ATTACHMENT_DATA) {
  ...
  function applyValueConverter(fieldName, fieldTypeMapped, fieldValue) {
    return fieldTypeMapped === 'binary' && fieldName === 'bodyHTML' ? convertUint8ArrayToString(fieldValue) : fieldValue;
  }

  function getFieldType(fieldProperty) {
    return fieldProperty.name.slice(16, 20).toLowerCase();
  }
  ...
  function readDataByBlockSmall(ds, msgData, startBlock, blockSize, dataTypeExtractor) {
    var byteOffset = startBlock * MSG.SMALL_BLOCK_SIZE;
  ...
  function readChainDataByBlockSmall(ds, msgData, fieldProperty, chain, dataTypeExtractor) {
    ...
    for (var i = 0, idx = 0; i < chain.length; i++) {
      var data = readDataByBlockSmall(ds, msgData, chain[i], MSG.SMALL_BLOCK_SIZE, extractorFieldValue.sbat.dataType.binary);
  ...
  function getChainByBlockSmall(ds, msgData, fieldProperty) {
    ...
    while (nextBlockSmall !== MSG.END_OF_CHAIN) {
  ...
  function getFieldValue(ds, msgData, fieldProperty, typeMapped) {
    ...
    var valueExtractor =
      fieldProperty.sizeBlock < MSG.BIG_BLOCK_MIN_DOC_SIZE ? extractorFieldValue.sbat : extractorFieldValue.bat;
  ...
  function convertUint8ArrayToString(uint8ArraValue) {
    try {
      return new TextDecoder('utf-8', {fatal: true}).decode(uint8ArraValue);
    } catch {}
  }
  ...
  MSGReader.prototype = {
    ...
    getAttachment: function (attach) {
      ...
      var fieldTypeMapped = MSG.FIELD.TYPE_MAPPING[getFieldType(fieldProperty)];
